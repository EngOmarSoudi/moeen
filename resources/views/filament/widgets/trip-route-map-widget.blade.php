<x-filament-widgets::widget>
    <div 
        x-data="{
            ...
            initMap() {
                const mapId = 'trip-route-map-{{ $this->getId() }}';
            ...
            renderTrips() {
                if (!this.trips || this.trips.length === 0) {
                    this.map.setView([24.7136, 46.6753], 6); // Center Riyadh
                    return;
                }
                ...
                this.trips.forEach(trip => {
                    if (trip.origin_lat && trip.origin_lng) {
                                ...
                                <div class='p-2 min-w-[150px]'>
                                    ...
                                    <div class='text-xs border-t pt-2 mt-2'>
                                        <div class='flex justify-between mb-1'>
                                            ...
                                            <span class='font-medium truncate'>${trip.origin_name || 'Pickup'}</span>
                                        </div>
                                        <div class='flex justify-between'>
                                            <span class='text-gray-500'>To:</span>
                                            <span class='font-medium truncate'>${trip.destination_name || 'Drop-off'}</span>
                                        </div>
                                    </div>
                                </div>
                            `)
                        ...
                        // Driver/Vehicle Marker (if tracking exists)
                        if (trip.current_lat && trip.current_lng) {
                            const driverIcon = L.divIcon({
                                className: 'custom-premium-marker',
                                html: `<div class='relative flex items-center justify-center'>
                                        <div class='absolute w-10 h-10 rounded-full animate-ping opacity-30' style='background-color: #f59e0b'></div>
                                        <div class='relative w-8 h-8 rounded-full bg-white shadow-xl flex items-center justify-center border-2 border-amber-500'>
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 text-amber-600">
                                              ...
                                              <path d="M8.25 19.5a1.5 1.5 0 10-3 0 1.5 1.5 0 003 0zM15.75 6.75a.75.75 0 00-.75.75v11.25c0 .414.336.75.75.75h2.25c.414 0 .75-.336.75-.75V7.5c0-.414-.336-.75-.75-.75h-2.25z" />
                                            </svg>
                                        </div>
                                       </div>`,
                                iconSize: [40, 40],
                            ...
                            const driverMarker = L.marker([trip.current_lat, trip.current_lng], { icon: driverIcon })
                            ...
                            // Draw line to driver if off-route or just to visualize progress? 
